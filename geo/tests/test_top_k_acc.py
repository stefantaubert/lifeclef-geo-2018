...
from geo.metrics.top_k_acc import top_k_acc
from geo.metrics.top_k_acc import top_k_accuracy

class TestGetRanksMethod(unittest.TestCase):
    '''Class for tests of the method get_score() and top_k_acc() of the module top_k_acc.'''
    def test_basic(self):
        y_pred = [
            [0.1, 0.2, 0.3, 0.4],
            [0.1, 0.2, 0.3, 0.4],
        ...
        self.assertEqual(0.25, top_k_acc(y_pred, y_true, class_map, 1))
        self.assertEqual(0.5, top_k_acc(y_pred, y_true, class_map, 2))
        self.assertEqual(0.75, top_k_acc(y_pred, y_true, class_map, 3))
        self.assertEqual(1, top_k_acc(y_pred, y_true, class_map, 4))
    ...
    def test_extrem(self):
        ...
        top_k_acc(y_pred, y_true, class_map, 1)
